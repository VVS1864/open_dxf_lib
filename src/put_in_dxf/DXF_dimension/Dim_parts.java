...
public class Dim_parts {
	...
	public Dim_parts(DXF_dimension dim) {
		...
		if (dim.angle == 90) {
			...
			dim.text_x = t[0];
			dim.text_y = t[1];
			lines.put("angle_arrow_1", 270.0);
			lines.put("angle_arrow_2", 90.0);
		} else {
			rotate = false;
			lines.put("angle_arrow_1", 0.0);
			lines.put("angle_arrow_2", 180.0);
		}
		// Find min and max x
		double x_min;
		double x_max;
		...
		if (dim.dim_text_change == 1) {
			...
			if (text_x == (x_max + x_min) / 2.0) {
				dim.dim_text_change = 1;
			}
		}
		...
		if (dim.arrow_type.equals("Arch")) {
			...
			if (dim_measure < s * 3.0)
				arrow_sign = -1;
			put_line("arrow_1", x_min, dim.y3, x_min + s * arrow_sign, dim.y3 + s / 10.0);
			put_line("arrow_2", x_min, dim.y3, x_min + s * arrow_sign, dim.y3 - s / 10.0);
			put_line("arrow_3", x_max, dim.y3, x_max - s * arrow_sign, dim.y3 + s / 10.0);
			put_line("arrow_4", x_max, dim.y3, x_max - s * arrow_sign, dim.y3 - s / 10.0);
		}
	...
	public void put_point(String name, double x1, double y1) {
		...
		lines.put(name + "_x", x1);
		lines.put(name + "_y", y1);
	}
	...
	public void put_line(String name, double x1, double y1, double x2, double y2) {
		...
		lines.put(name + "_x1", x1);
		lines.put(name + "_x2", x2);
		lines.put(name + "_y1", y1);
		lines.put(name + "_y2", y2);
